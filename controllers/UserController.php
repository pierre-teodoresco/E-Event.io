...
final class UserController{
    ...
    //référence à la classe modele associée
    private $model;
    ...
    public function login(){
        ...
        //Si l'utilisateur a soumis un formulaire de connection
    ...
    public function register(){
        ...
        //grille de données qui sera fournie lors de l'affichage de la page
        $registerData = [
            'email' => '',
            'rank' => '',
            'emailError' => '',
        ...
        //Si l'étudiant a fourni un formulaire d'enregistrement
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            $this->model->checkRegister(htmlentities(htmlspecialchars($registerData['email'])), $password, $options, $registerData['rank']);
            $this->sendEmailPassword($registerData['email'], $password);
            $registerData['loginData'] = '<p style=\'color:green\'>Félicitations, regardez votre email pour avoir vos identifiants de connexion. (Pensez à vérifier vos spams)</p>';
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST' ){
            if($_POST['action'] == "usertable"){
                ...
                foreach ($users as $user){
                    $this->model->updateOne('point', $admin_data['campagneUser'], $user['id']);
                }
    ...
    public function jury(){
        ...
        //Le jury a mis a jour la selection du jury
        if($_SERVER['REQUEST_METHOD'] == 'POST' ){
            $event2 = $eventModel->getAllEvent();
            $i = 0;
            foreach ($event2 as $event){
                if($_POST['event-select'][$i] == 'on') $eventModel->updateOne('jury', '1', $event['id']);
                else $eventModel->updateOne('jury', '0',$event['id']);
                $i++;
            }
        }
        ...
        foreach ($events as $event){
            $dataValues .= "<tr><td><input name=\"event-select[]\" id=\"event-select\" type='checkbox' ";
            if($event[jury]==1) $dataValues .="checked";
            $dataValues .= "></td><td>$event[id]</td><td>$event[title]</td><td>$event[votes]</td></tr>";
        }
        $jury_data['tableUsers'] = $dataValues;
